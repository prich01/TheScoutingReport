...
async function handleSaveGameLog() {
  ...
  try {
    ...
    console.error("Game Log Processing Error:", error);
    if (statusEl) statusEl.textContent = "❌ Error processing game log.";
    alert(`Failed to process log: ${error.message}`);
  }
}
...
function renderOpponentReport() {
  ...
  if (pitcherList.length === 0) {
    pitcherContainer.innerHTML = `<p class="text-xs text-slate-500 italic col-span-full">No pitcher substitutions detected in logs.</p>`;
  } else {
          ...
          <div class="flex justify-between items-start border-b border-slate-800 pb-3">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-xs font-bold text-amber-400 bg-amber-950/80 border border-amber-800/60 px-2 py-0.5 rounded">#${p.number || '00'}</span>
                <h3 class="text-base font-bold text-white">${p.name}</h3>
              </div>
              <p class="text-[11px] text-slate-400 mt-1">Role: <span class="text-slate-200 font-semibold">Pitcher</span> | Throws: <span class="text-slate-200 font-semibold">${p.throws || 'R'}</span></p>
            </div>
          </div>
      ...
      card.innerHTML = `
        <div>
          <div class="flex justify-between items-start border-b border-slate-800 pb-3">
            <div>
              <div class="flex items-center gap-2">
                <span class="text-xs font-bold text-emerald-400 bg-emerald-950/80 border border-emerald-800/60 px-2 py-0.5 rounded">#${hitter.number || '00'}</span>
                <h3 class="text-base font-bold text-white">${hitter.name}</h3>
              </div>
              <p class="text-[11px] text-slate-400 mt-1">Pos: <span class="text-slate-200 font-semibold">${hitter.pos || 'DH'}</span> | Bats: <span class="text-slate-200 font-semibold">${hitter.bats || 'R'}</span> | Throws: <span class="text-slate-200 font-semibold">${hitter.throws || 'R'}</span></p>
            </div>
          </div>
...
function renderDashboard() {
  ...
  const games = getOpponentGames(activeOpponent);
  const roster = getOpponentRoster(activeOpponent);
  if (badgeEl) badgeEl.textContent = `${games.length} ${games.length === 1 ? 'Game' : 'Games'} Tracked`;
  ...
  calloutsGrid.innerHTML = `
    <div class="bg-slate-900 border border-emerald-900/60 p-4 rounded-xl shadow-lg relative overflow-hidden">
      ...
      <div class="text-base font-bold text-white flex items-center gap-2">
        <span>#${topHitter ? (topHitter.number || '00') : '00'}</span>
    ...
    <div class="bg-slate-900 border border-amber-900/60 p-4 rounded-xl shadow-lg relative overflow-hidden">
      ...
      <div class="text-base font-bold text-white flex items-center gap-2">
        <span>#${powerBat ? (powerBat.number || '00') : '00'}</span>
    ...
    <div class="bg-slate-900 border border-sky-900/60 p-4 rounded-xl shadow-lg relative overflow-hidden">
      ...
      <div class="text-base font-bold text-white flex items-center gap-2">
        <span>#${acePitcher ? (acePitcher.number || '00') : '00'}</span>
  ...
  hitterList.forEach(h => {
    ...
    tr.innerHTML = `
      <td class="p-2.5 font-bold text-amber-400">#${h.number || '00'}</td>
      <td class="p-2.5 font-bold text-white">${h.name}</td>
      <td class="p-2.5 text-slate-400">${h.pos || 'DH'}</td>
      <td class="p-2.5 text-slate-400">${h.bats || 'R'}/${h.throws || 'R'}</td>
